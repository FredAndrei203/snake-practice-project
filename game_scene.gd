...
func _create_new_game(play_area_size: Vector2, tick_rate: float, apple_count: int) -> void:
	main_menu.enter_the_game.disconnect(_on_main_menu_ui_enter_the_game)
	main_menu.queue_free()
	var new_game: GameProper = game_proper_scene.instantiate()
	add_child(new_game)
	new_game.game_session_done.connect(_on_game_proper_game_session_done)
	new_game._initialize_round(play_area_size, tick_rate, apple_count)
	game_proper = new_game
	game_proper.start_round()

func _create_menu() -> void:
	game_proper.game_session_done.disconnect(_on_game_proper_game_session_done)
...
func _on_game_proper_game_session_done() -> void:
	_create_menu()
